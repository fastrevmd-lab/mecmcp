...
impl SessionTracker {
    ...
    pub fn try_register(&self, id: SessionId, now: Instant) -> bool {
        ...
        if self.max_sessions > 0 && prev >= self.max_sessions {
            ...
            metrics::record_limit_hit("session_cap", "session_registration_rejected");
            return false;
        }
        ...
        if let Some(names) = metrics::metric_names() {
            ::metrics::gauge!(names.active_sessions.clone()).increment(1.0);
        }
        true
    }
    ...
    fn unregister_inner(&self, id: &SessionId) -> bool {
        ...
        if removed {
            self.active.fetch_sub(1, Ordering::AcqRel);
            if let Some(names) = metrics::metric_names() {
                ::metrics::gauge!(names.active_sessions.clone()).decrement(1.0);
            }
        }
        let mut state = self.token_state();
...
fn finish_reap(tracker: &SessionTracker, expired: ExpiredSession) {
    if tracker.unregister_inner(&expired.id) {
        if let Some(names) = metrics::metric_names() {
            ::metrics::counter!(names.sessions_reaped.clone(), "reason" => expired.reason.as_str())
                .increment(1);
        }
