...
/// Defect 4 sabotage: without the check, edited reason DOES get promoted to v3 digest.
///
/// This test demonstrates the laundering: load a v2 file with edited reason,
/// write it back (which stamps v3), and confirm the v3 digest includes the
/// unauthenticated text. This is the bug being fixed.
#[test]
#[cfg_attr(not(feature = "sabotage"), ignore)]
fn defect_4_sabotage_edited_reason_gets_promoted_to_v3_digest() {
    // Same setup as above
    let dir = tempfile::tempdir().expect("tempdir");
    let state_path = dir.path().join("state.json");
    ...
    let waiver = WaiverRecord {
        kind: WaiverKind::LabMode,
        reason: "edited-by-attacker".to_owned(),
        expires_at_unix: None,
        ticket: None,
    };
    ...
    // TEMPORARILY REMOVE THE CHECK to demonstrate the laundering
    // In the sabotage test, we're testing WITHOUT the fix, so this SHOULD
    // succeed with the check commented out in persistence.rs
    let loaded =
        read_state(&state_path, 8 * 1024 * 1024).expect("load should succeed without the check");

    // Write it back (which stamps v3)
    write_state(&state_path, &loaded, 8 * 1024 * 1024).expect("write v3");

    // Read the raw JSON to inspect the digest
    let raw = std::fs::read_to_string(&state_path).expect("read raw");
    let on_disk: serde_json::Value = serde_json::from_str(&raw).expect("parse");
    assert_eq!(on_disk["version"], 3, "File should be v3 now");

    // Extract the approval digest
    let approval = &on_disk["state"]["change_sets"][&change_set_id]["approval"];
    let written_digest = approval["digest"].as_str().expect("digest");

    // Compute what the v3 digest SHOULD be over "edited-by-attacker"
    use mecmcp_changeset::digest::compute_waiver_digest_v3;
    let expected_v3_digest = compute_waiver_digest_v3(
        &change_set_id,
        &digest,
        &owner,
        approved_at,
    ...
    assert_eq!(
        written_digest, expected_v3_digest,
        "The v3 digest now covers the unauthenticated 'edited-by-attacker' text — laundering complete"
    );
}
...
/// Defect 5 sabotage: without the check, load-write-load corrupts the file.
///
/// This test demonstrates the corruption: load a v2 file with both approver and
/// waived (passes validation because it takes the approver branch), write it back
/// (the migration overwrites the approver digest with a waiver digest), and
/// confirm the next load fails.
#[test]
#[cfg_attr(not(feature = "sabotage"), ignore)]
fn defect_5_sabotage_load_write_load_corrupts_the_file() {
    let dir = tempfile::tempdir().expect("tempdir");
    let state_path = dir.path().join("state.json");
    ...
    // Load (passes validation because approver digest is valid)
    let loaded = read_state(&state_path, 8 * 1024 * 1024)
        .expect("load should succeed without the mutual exclusion check");

    // Write (the migration overwrites approver digest with waiver digest)
    write_state(&state_path, &loaded, 8 * 1024 * 1024).expect("write");

    // Load again — MUST FAIL with approval digest mismatch
    let result = read_state(&state_path, 8 * 1024 * 1024);
    assert!(
        result.is_err(),
        "Second load should fail with digest mismatch"
    );
    let error_message = result.unwrap_err().to_string();
    assert!(
